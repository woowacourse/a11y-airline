...
document.getElementById('subPassengerButton').addEventListener('click', () => {
  ...
  if (value <= 1) {
    politeText.textContent = '1 미만으로 감소할 수 없어용';
  } else {
...
document.getElementById('addPassengerButton').addEventListener('click', () => {
  ...
  if (value >= 3) {
    politeText.textContent = '3 초과로 추가할 수 없어용';
  } else {
    passengerCount.value = value + 1;
...
passengerCount.addEventListener('change', () => {
  ...
  if (!value || value < 1) {
    ...
    politeText.textContent =
      '1 미만으로 감소할 수 없어용, 성인 1 텍스트 숫자만 수정';
  } else if (value > 3) {
    passengerCount.value = 3;
    politeText.textContent =
      '3 초과로 추가할 수 없어용, 성인 3 텍스트 숫자만 수정';
  } else {
